    ...
    try {
      ...
      if (list.length === 0 && projectId === 1) {
        ...
        return NextResponse.json({
          success: true,
          data: (raw as any).rows.map(mapLegacyOrder),
          recovered: true,
        });
      }
      ...
      const data = projectId === 1 ? all : all.filter((r: any) => r.projectId === projectId);
      return NextResponse.json({ success: true, data, recovered: true });
    }
  } catch (error: any) {
