...
function mapLegacyOrder(r: any, includeImages = true) {
  return {
    id: Number(r.id), projectId: Number(r.project_id ?? 1), name: r.name,
    imageUrl: includeImages ? (r.image_url ?? null) : null, itemUrl: r.item_url ?? null, forWhom: r.for_whom ?? null,
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (list.length === 0 && projectId === 1) {
      ...
      return NextResponse.json({ success: true, data: (raw as any).rows.map((r: any) => mapLegacyOrder(r, includeImages)), recovered: true }, {
        headers: { "Cache-Control": "private, max-age=5, stale-while-revalidate=20" }
      });
    }
    const data = includeImages ? list : list.map((o: any) => ({ ...o, imageUrl: null }));
    ...
    try {
      const raw = await db.execute(sql`SELECT * FROM "orders" ORDER BY "id" DESC`);
      const all = (raw as any).rows.map((r: any) => mapLegacyOrder(r, includeImages));
