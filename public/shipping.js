(function () {
  ...
  async function installInlineTable(){
    const table=findOrdersTable();if(!table)return;const {heads,map}=headerMap(table);
    ...
    Array.from(table.tBodies[0]?.rows||[]).forEach(row=>{
      ...
      if(weightIdx>=0)weightInput=makeInput(cells[weightIdx],o.id,"weight",o.weight||0,"number",{min:"0",step:"0.01"});
      if(priceIdx>=0)makeInput(cells[priceIdx],o.id,"priceCny",o.priceCny||0,"number",{min:"0",step:"0.01"});
      if(chinaIdx>=0){const auto=o.shippingChinaUsd==null||Number(o.shippingChinaUsd)===0;usdInput=makeInput(cells[chinaIdx],o.id,"shippingChinaUsd",auto?((o.weight||0)*usdPerKg()):o.shippingChinaUsd,"number",{min:"0",step:"0.01"});if(!auto)usdInput.dataset.manual="1";usdInput.addEventListener("input",()=>usdInput.dataset.manual="1");if(weightInput)weightInput.addEventListener("input",()=>{if(usdInput.dataset.manual!=="1")setReactInputValue(usdInput,(Number(weightInput.value||0)*usdPerKg()).toFixed(2));});}
      if(rbIdx>=0)makeInput(cells[rbIdx],o.id,"shippingBelarusByn",o.shippingBelarusByn||0,"number",{min:"0",step:"0.01"});
    });
  }

  function installSettings(){if(document.getElementById("cargo-usd-settings"))return;const buttons=Array.from(document.querySelectorAll("button"));const anchor=buttons.find(b=>(b.textContent||"").includes("Изменить курс"))||buttons.find(b=>(b.textContent||"").includes("Экспорт"));if(!anchor?.parentElement)return;const box=document.createElement("div");box.id="cargo-usd-settings";box.style.cssText="display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:5px 8px;margin-left:6px;border-radius:10px;background:#0f172a;border:1px solid #334155;color:#94a3b8;font:700 10px sans-serif";box.innerHTML='<span style="color:#fbbf24">🚚 Китай → РБ</span><label>$ / кг <input id="cargo-usd-kg" type="number" min="0" step="0.01" style="width:55px;background:#020617;color:#fb923c;border:1px solid #334155;border-radius:6px;padding:3px;text-align:center"></label><label>USD→BYN <input id="cargo-usd-byn" type="number" min="0" step="0.0001" style="width:62px;background:#020617;color:#22d3ee;border:1px solid #334155;border-radius:6px;padding:3px;text-align:center"></label>';anchor.parentElement.appendChild(box);const a=box.querySelector("#cargo-usd-kg"),b=box.querySelector("#cargo-usd-byn");a.value=usdPerKg();b.value=usdByn();a.addEventListener("input",()=>{setNum(USD_PER_KG_KEY,a.value);installInlineTable();});b.addEventListener("input",()=>setNum(USD_BYN_KEY,b.value));}
  ...
  let timer=null;const observer=new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(init,200);});const start=()=>{if(document.body)observer.observe(document.body,{childList:true,subtree:true});init();};if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",start);else start();
})();