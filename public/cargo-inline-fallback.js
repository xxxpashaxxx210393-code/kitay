  ...
  if(url.endsWith('/api/orders/inline') && init && String(init.method||'GET').toUpperCase()==='PUT'){
    ...
    if(body.id&&body.field){
      try{
        ...
        if(fallback.ok){const data=await fallback.json().catch(()=>({}));return new Response(JSON.stringify({success:true,data:data.data||data}),{status:200,headers:{'Content-Type':'application/json','Cache-Control':'no-store'}});}
      }catch{}
    }
